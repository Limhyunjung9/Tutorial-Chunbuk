...
void main(void)                 
{
    ...
    {
        input[i]=i;             //input = 메모리상의 주소 
    }

    /* for checking call by reference (call by reference 관점에서 보는 것이 중요) */
    printf("address of input = %p\n", input);  //input배열의 주소를 출력

    answer = sum(input,MAX_SIZE);              //answer에 함수 sum을 실행한 결과를 대입, input=주소 
...
float sum(float list[], int n )                 //list는 input의 주소를 담고있음 
{
    ...
    for(i=0; i<n; i++)                           
       tempsum += list[i];                       //위에서 for문을 통 해 input[i]=i라고 선언했기 때문에 tempsum은 0~n-1까지의 합. main()에선 n 자리에 MAX_SIZE가 오기 때문에 0~99의 합
